...
class Log:
    ...
    def train(self, len_dataset: int) -> None:
        ...
        self.is_train = True
        self.last_steps_state = {"loss": 0.0, "accuracy": 0.0, "steps": 0, "sharpness": 0}
    ...
    def __call__(self, model, loss, accuracy, learning_rate: float = None, sharpness=0) -> None:
        if self.is_train:
            self._train_step(model, loss, accuracy, learning_rate, sharpness=sharpness)
        else:
    ...
    def flush(self) -> None:
        ...
        if self.is_train:
            ...
            sharpness = self.epoch_state["sharpness"] / self.epoch_state["steps"]

            print(
                f"\r┃{self.epoch:12d}  ┃{loss:12.4f}  │{100*accuracy:10.2f} %  │{sharpness:12.4f}  ┃{self.learning_rate:12.3e}  │{self._time():>12}  ┃",
                end="",
                flush=True,
            )
    ...
    def _train_step(self, model, loss, accuracy, learning_rate: float, sharpness=0) -> None:
        self.learning_rate = learning_rate
        self.last_steps_state["loss"] += loss.sum().item()
        self.last_steps_state["accuracy"] += accuracy.sum().item()
        self.last_steps_state["sharpness"] += sharpness.sum().item()
        ...
        self.epoch_state["sharpness"] += sharpness.sum().item()
        self.step += 1
        ...
        if self.step % self.log_each == self.log_each - 1:
            ...
            sharpness = self.last_steps_state["sharpness"] / self.last_steps_state["steps"]

            self.last_steps_state = {"loss": 0.0, "accuracy": 0.0, "steps": 0, "sharpness": 0}
            progress = self.step / self.len_dataset

            print(
                f"\r┃{self.epoch:12d}  ┃{loss:12.4f}  │{100*accuracy:10.2f} %  │{sharpness:12.4f}  ┃{learning_rate:12.3e}  │{self._time():>12}  {self.loading_bar(progress)}",
                end="",
                flush=True,
            )
            # print(
    ...
    def _reset(self, len_dataset: int) -> None:
        ...
        self.epoch_state = {"loss": 0.0, "accuracy": 0.0, "steps": 0, "sharpness": 0}
    ...
    def _print_header(self) -> None:
        ...
        print(f"┃              ┃              ╷              ╷              ┃              ╷              ┃              ╷              ┃")
        print(f"┃       epoch  ┃        loss  │    accuracy  │   Sharpness  ┃        l.r.  │     elapsed  ┃        loss  │    accuracy  ┃")
        print(f"┠──────────────╂──────────────┼──────────────┼──────────────╂──────────────┼──────────────╂──────────────┼──────────────┨")
