"""
Ref: https://github.com/SamsungLabs/pytorch-ensembles/blob/76b71c9ea3886e3a7fece9ddc3740e26f64ecbaa/metrics.py
...
def acc_aac(preds, targets, steps=10, return_plot=False, **args):
...
def get_ece(preds, targets, n_bins=15, return_avg_conf=False, **args):
    ...
    for bin_lower, bin_upper in zip(bin_lowers, bin_uppers):
        ...
        if prop_in_bin > 0:
            ...
            avg_confs_in_bins.append(np.nan)
    ...
    for runs in range(n_runs):
        ...
        for i, (tr_idx, te_idx) in enumerate(KFold(n_splits=n_splits, shuffle=True).split(log_preds)):
            if temp_scale:
                ...
                test_lp = log_preds[te_idx]
            pre_exp.append(np.exp(test_lp))
            target_exp.append(targets[te_idx])
        pre_exp = np.concatenate(pre_exp)
        target_exp = np.concatenate(target_exp)
        ece, _, avg_confs = get_ece(pre_exp, target_exp, return_avg_conf=True, **args)
        runs_stat_ece.append(ece)
        runs_stat_conf.append(np.asarray(avg_confs))

    # if verbose:
    #     print("Conf - Acc", avg_confs)

    return sum(runs_stat_ece)/len(runs_stat_ece), sum(runs_stat_conf)/len(runs_stat_conf)