...
class BasicUnit(nn.Module):
    def __init__(self, channels: int, dropout: float, inplace=False):
        super(BasicUnit, self).__init__()
        self.block = nn.Sequential(OrderedDict([
            ("0_normalization", nn.BatchNorm2d(channels)),
            ("1_activation", nn.ReLU(inplace=inplace)),
            ("2_convolution", nn.Conv2d(channels, channels, (3, 3), stride=1, padding=1, bias=False)),
            ("3_normalization", nn.BatchNorm2d(channels)),
            ("4_activation", nn.ReLU(inplace=inplace)),
            ("5_dropout", nn.Dropout(dropout, inplace=inplace)),
...
class DownsampleUnit(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, stride: int, dropout: float, inplace=False):
        ...
        self.block = nn.Sequential(OrderedDict([
            ...
            ("2_activation", nn.ReLU(inplace=inplace)),
            # ("3_dropout", nn.Dropout(dropout, inplace=True)),
            ("3_dropout", nn.Dropout(dropout, inplace=inplace)),
...
class WideResNet(nn.Module):
    def __init__(self, depth: int, width_factor: int, dropout: float, in_channels: int, labels: int, inplace=False):
        ...
        self.f = nn.Sequential(OrderedDict([
            ...
            ("5_activation", nn.ReLU(inplace=inplace)),
